...
**Dependencies:**
- `RandomDetector`, `StoppedDetector`, `OffTrackDetector`, `MeatballDetector`, `TowDetector` - Individual detectors
...
**Key Types:**
- `DetectorEventTypes(Enum)` - Event type identifiers (RANDOM, STOPPED, OFF_TRACK, MEATBALL, TOWING)
...
### `src/core/detection/tow_detector.py` (65 lines)
**Role:** Detects cars that towed (teleported) to pits via iRacing's tow interface

**Key Logic:**
- Compares `track_loc` between current and previous frames (uses double-buffering)
- Tow signature: car jumps directly from `on_track`/`off_track` to `in_pit_stall` without passing through `aproaching_pits`
- Returns the **previous** driver object so `lap_distance` reflects the incident location, not the pit stall
- Filters out pace car, negative laps_completed, not_in_world, and cars already on pit road
- Currently configured as shadow-only (threshold=99999, weight=0.0)
...
**When to Modify:**
- Adjusting tow detection criteria
- Enabling for actual safety car triggering
